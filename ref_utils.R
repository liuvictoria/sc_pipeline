...
MyPackages <- c(
  ...
  "grid", "lattice", "gtools", "Biobase", "RColorBrewer", "tibble",
  "Seurat", "cowplot", "patchwork", "stringr", "ComplexHeatmap", 
  "SingleCellExperiment", "ggmin", "Cairo",
...
config <- fromJSON(file = paste0(args[1],"/",args[2],"/config.json"))
analyses <- fromJSON(file = paste0(args[1],"/",args[2],"/analysis.json"))
master <- fromJSON(file = paste0("/projects/compsci/jgeorge/USERS/alizae/GBM/matching_patients/R_Code/sc_pipeline/master.json"))
...
write_experimental_configs <- function(suffix = "", code_file) {
  ...
  file.copy(
    from = paste0(args[1],"/",args[2],"/config.json"), to = config_params_filename
  )
  ...
  file.copy(
    from = paste0(args[1],"/",args[2],"/analysis.json"), to = analysis_params_filename
  )
  ...
  file.copy(
    from = paste0(args[1],"/",code_file, ".R"), to = code_filename
  )
  ...
  file.copy(
    from = paste0(args[1],"/ref_utils.R"), to = utils_filename
  )
}

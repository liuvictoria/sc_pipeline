...
gm_auth_configure(
  path = paste0(
    "~/Box/Yun\ lab\ projects/scRNAseq\ Project/",
    "2808combined\ old\ and\ new/Victori2808aAnalysis/",
    "Rbotcredentials.json"
  )
)
...
for(i in 1:length(FILES)){
  ...
  print(head(SeuratObjMYSC@meta.data)[5])
  ...
  # rename columns
  colnames(SeuratObjMYSC@meta.data)[7] <- "pANN"
  colnames(SeuratObjMYSC@meta.data)[8] <- "DoubletStatus"
...
if (DefaultAssay(SeuratObj) != "RNA") {
  DefaultAssay(SeuratObj) <- "RNA"
}
# SCT for GEX
if (SCT) {
  SeuratObj <- SCTransform(SeuratObj, method = "glmGamPoi", verbose = FALSE)
} else {
  SeuratObj <- SeuratObj %>% 
    NormalizeData() %>%
    FindVariableFeatures() %>%
...
if (config$run_Harmony_CC_controls) {
  ...
  SeuratObj <- SeuratObj %>% 
    RunHarmony(config$batch_norm_by) %>%
...
SeuratObj <- SeuratObj %>% 
  ...
  RunHarmony(config$batch_norm_by)
E1 <- ElbowPlot(SeuratObj, ndims = 15, reduction = "harmony")
...
samples_list <- SplitObject(SeuratObj, split.by = config$batch_norm_by) %>%
  lapply(
    FUN = function(x) {
      x <- NormalizeData(
        x, normalization.method = "CLR", margin = 2)
    }
  )
# select features that are repeatedly variable across datasets
features <- SelectIntegrationFeatures(
  object.list = samples_list
)
# find anchor cells (this step stakes a long time)
cell_anchors <- FindIntegrationAnchors(
  object.list = samples_list, anchor.features = features)
# combined into integrated Seurat object
...
DefaultAssay(SeuratObj_ADT) <- "integrated"
...
# ignore the warnings about offending keys.
SeuratObj <- RenameAssays(SeuratObj, ADT = "ADTpre")
SeuratObj[["integrated"]] <- SeuratObj_ADT[["integrated"]]
SeuratObj[["pcaADT"]] <- SeuratObj_ADT[["pcaADT"]]
SeuratObj <- RenameAssays(SeuratObj, integrated = "ADT")
...
SeuratObj <- SeuratObj %>%
  FindNeighbors(reduction = "harmony", dims = c(1:8)) %>%
  FindClusters(resolution = RESOLUTION)

######## RUN UMAP ########
SeuratObj <- RunUMAP(SeuratObj, reduction = "harmony", dims = c(1:8))

# batch effects should be miminal at this point
...
file.copy(
  from = here("config.json"), to = paste0(ConfigDirectory, "params.json")
)
sink(type = "output")
